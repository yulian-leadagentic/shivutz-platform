...
async def register_corporation(
    ...
    # R30 §15 · same gap as the contractor side — only the team-INVITE
    # route (:636) normalized; registration wrote contact_phone raw.
    # Normalized before the duplicate guard so the membership_request
    # branch stores the canonical form too.
...
async def invite_corporation_user(
    ...
    # U8 §4c — server-side border. The frontend now sends the
    # normalized form (checkIsraeliPhone().normalized), but this
    # route is directly reachable, and stored `invited_phone` used
    # to accept "0525267879גגג" verbatim then propagate it to the
    # team.invited SMS payload. Reject unless the input strictly
    # matches an Israeli mobile pattern.
    try:
    ...
    try:
        ...
        # R4 · seat gate reads effective_seats (same helper contractors
        # use). Corp plans still have extra_user_price_nis NULL — no
        # extra-seat purchase — so a corp above `total` always gets
        # 402 seat_limit, not seat_upgrade_required. Admin grants can
        # push total above max_users; that's intentional (R4 §3a).
        try:
            eff = effective_seats(org_id, "corporation")
        except httpx.HTTPError:
            raise HTTPException(status_code=503, detail="entitlement_service_unreachable")
        limits = tier_limits(eff["tier"], "corporation")
...
class TeamMemberPatch(BaseModel):
    ...
    # backfill email also re-enables the email channel checkbox in the
...
# Toggle which members are exposed to the other party as a contact
# point on approved deals. The corp deal page renders the contractor's
# deal contacts; mirror endpoint on the contractor router.

class DealContactPatch(BaseModel):
    is_deal_contact: bool


@router.patch("/{org_id}/users/{membership_id}/deal-contact")
def set_corporation_deal_contact(
    org_id: str,
    membership_id: str,
    data: DealContactPatch,
    x_user_id:   Optional[str] = Header(default=None),
    x_user_role: Optional[str] = Header(default=None),
):
...
def list_corporation_deal_contacts(org_id: str):
    """Public-ish read used by deal screens — returns the active list
    of people the OTHER party (a contractor on a deal with this corp)
    should call. No auth gate beyond the deal context — by the time a
    contractor is rendering this they've already passed reveal and the
    corp's identity is unlocked anyway."""
    return team_mgmt.list_deal_contacts("corporation", org_id)
