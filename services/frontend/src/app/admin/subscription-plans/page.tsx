                ...
                return (
                  <div key={p.id} className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm space-y-3">
                    ...
                    <button
                      ...
                      onClick={() => save(p)}
                      disabled={busy === p.id}
                      className="w-full mt-2 bg-brand-600 hover:bg-brand-700 text-white text-sm font-semibold py-2 rounded-lg
      ...
      <div className="flex items-center gap-2">
        <input
          ...
          className={`w-full border rounded-lg px-2 py-1.5 text-sm outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500 ${
            isUnlimited ? 'border-brand-200 bg-brand-50/50 placeholder:text-brand-700' : 'border-slate-300'
          }`}
