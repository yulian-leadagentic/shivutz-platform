...
import { authApi, orgApi, enumApi } from '@/lib/api';
...
export default function RegisterContractorPage() {
  ...
  function toggleRegion(code: string) {
    setStep2((prev) => ({
      ...prev,
      operating_regions: prev.operating_regions.includes(code)
        ? prev.operating_regions.filter((r) => r !== code)
        : [...prev.operating_regions, code],
    }));
  }
  ...
  function validateStep2(): string {
    if (!step2.company_name_he.trim()) return 'יש להזין שם חברה בעברית';
    if (!step2.company_name.trim()) return 'יש להזין שם חברה באנגלית';
