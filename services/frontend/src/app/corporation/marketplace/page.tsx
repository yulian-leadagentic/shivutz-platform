      ...
      <Card>
        ...
        <CardContent className="p-0">
          {loading ? (
            ...
            <>
              {/* Mobile card-view (≤sm). Icon-only 7×7 actions in the
                  desktop table are unreachable on touch — mobile gets
                  labeled 44×44 buttons. Rows kept 1-per-card, key
                  fields inline, status stays a labeled pill. */}
                  ...
                  return (
                    <li key={l.id} className="p-4 space-y-3">
                      ...
                      <div className="flex items-center gap-2">
                        <Link href={`/corporation/marketplace/${l.id}/edit`} className="flex-1">
                          <Button size="sm" variant="outline" className="w-full min-h-11">
                            <Pencil className="h-4 w-4 me-2" />ערוך
                          </Button>
                        </Link>
                        <Button
                          ...
                          aria-label={l.status === 'active' ? 'השהה מודעה' : 'הפעל מודעה'}
