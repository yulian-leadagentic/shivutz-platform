...
import { Loader2, Send, FileText, Users, CheckCircle, XCircle, AlertTriangle } from 'lucide-react';
...
export default function CorporationDealPage() {
  ...
  async function loadAll() {
    try {
      const [d, msgs, ws] = await Promise.allSettled([
        dealApi.get(id),
        dealApi.messages(id),
        dealApi.workers(id),
      ]);
      if (d.status === 'fulfilled') setDeal(d.value);
      else setError('שגיאה בטעינת העסקה');
    ...
    <div className="space-y-6 max-w-5xl">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">נוצרה: {fmt(deal.created_at)}</p>
        </div>
            ...
            {!showRejectConfirm ? (
              <div className="flex items-start gap-4 flex-wrap">
                <div className="flex-1">
                  ...
                  <p className="text-amber-700 text-xs mt-0.5">
                    קבלן שלח הצעת עסקה עם {deal.workers_count} עובדים. בדוק את הפרטים ואשר או דחה.
                  </p>
                </div>
              ...
              <div className="flex items-center gap-4 flex-wrap">
                <div className="flex items-center gap-2 text-red-700">
                  ...
                  <span className="text-sm font-medium">האם לדחות את ההצעה? פעולה זו אינה הפיכה.</span>
                </div>
                <div className="flex gap-2">
                  <Button
                    size="sm"
                  ...
                  <Button size="sm" variant="ghost" onClick={() => setShowRejectConfirm(false)}>
                    ביטול
                  </Button>
                </div>
              </div>
                ...
                <tbody>
                  {workers.map((w) => (
                    <tr key={w.id} className="border-b border-slate-50 last:border-0">
                      ...
                      <td className="py-2 text-slate-600">{fmt(w.visa_valid_until)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            {reportSuccess ? (
              ...
              <form onSubmit={handleReport} className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <Input
                  ...
                  onChange={(e) => setReportForm((f) => ({ ...f, actual_workers: e.target.value }))}
                  required
                />
                <Input
                  ...
                  onChange={(e) => setReportForm((f) => ({ ...f, actual_start_date: e.target.value }))}
                  required
                />
                <Input
                  ...
                  onChange={(e) => setReportForm((f) => ({ ...f, actual_end_date: e.target.value }))}
                  required
                />
