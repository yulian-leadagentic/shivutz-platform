...
// Pivot/v2 Phase 3+ — search-first landing.
// The homepage IS the search experience. No login required to search
// or read ad content; only revealing corp contact info triggers login
// + subscription check.
...
export default function LandingSearchPage() {
  ...
  async function reveal(ad: AdSearchResult) {
    // Not logged in → send them to /login and remember where to come back.
    if (!isLoggedIn()) {
    ...
    try {
      ...
      if (/401|Unauthorized/i.test(msg)) {
        router.push(`/login?returnTo=${encodeURIComponent('/')}`);
      } else if (/402|subscription_required|tier_reveal_limit/i.test(msg)) {
        ...
        <main className="flex-1">
          ...
          <section className="bg-gradient-to-b from-slate-50 to-white pt-24 pb-10">
            <div className="max-w-3xl mx-auto px-4 space-y-5">
              ...
              {/* Example chips (only visible before first search) */}
              {!resp && !loading && (
