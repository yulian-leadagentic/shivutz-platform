...
export default function ContractorUsersPage() {
  ...
  useEffect(() => {
    ...
    if (!id) return;
    setOrgId(id);
    apiFetch<OrgUser[]>(`/organizations/contractors/${id}/users`)
      .then(setUsers)
      .catch(console.error)
  ...
  async function handleInvite() {
    ...
    try {
      const u = await apiFetch<OrgUser>(`/organizations/contractors/${orgId}/users`, {
        method: 'POST',
      ...
      {showForm && (
        <Card>
          <CardContent className="pt-4 space-y-3">
            ...
            <Input label="אימייל" type="email" placeholder="user@company.co.il" dir="ltr"
